...
class MapProvider extends ChangeNotifier {
  ...
  void loadObservations() async {
    ...
    for (ArtsTaxon taxon in _taxons) {
      ...
      var filteredObservations = firstPage.observations.where((m) => _shouldMakeMarker(m));
      _markers = _markers.union(filteredObservations.map((o) => _makeMarker(o)).toSet());
      ...
      for (int pageId = 1; pageId < firstPage.totalPages; pageId++){
        var page = await ArtsApi.getObservationPage(taxon: taxon, pageIndex: pageId);
        filteredObservations = page.observations.where((m) => _shouldMakeMarker(m));
        _markers = _markers.union(filteredObservations.map((o) => _makeMarker(o)).toSet());
        _progress = pageId/firstPage.totalPages;
        print(_progress);
        print("$_progress : ${pageId+1} out of ${firstPage.totalPages} loaded");
        notifyListeners();
      }
      _progress = null;
      _isLoading = false;
  ...
  Marker _makeMarker(Observations o) {
    return Marker(
      ...
      position: LatLng(double.parse(o.latitude.replaceAll(',', '.')), double.parse(o.longitude.replaceAll(',', '.')))
    );
  }
